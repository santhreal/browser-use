...
async def test_focus_vs_all_elements():
	...
	while True:
		...
		while True:
			...
			eval_representation = serialized_state.eval_representation()
			# print(eval_representation)

			encoding = tiktoken.encoding_for_model('gpt-4.1-mini')
			token_count = len(encoding.encode(eval_representation))
			print(f'Token count: {token_count}')

			os.makedirs('./tmp', exist_ok=True)
			async with await anyio.open_file('./tmp/eval_representation.txt', 'w', encoding='utf-8') as f:
				await f.write(eval_representation)

			print('Eval representation written to ./tmp/eval_representation.txt')

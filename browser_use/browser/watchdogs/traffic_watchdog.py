...
class TrafficWatchdog(BaseWatchdog):
	...
	def get_pending_requests(self, target_id: TargetID) -> list[dict[str, str | float]]:
		"""Get current pending network requests for a target.

		Args:
			target_id: The target ID to get pending requests for

		Returns:
			List of dicts with keys: request_id, url, resource_type, timestamp
		"""
		return list(self._pending_requests.get(target_id, {}).values())

	def is_network_stable(self, target_id: TargetID, idle_threshold: float = 0.5) -> bool:
		"""Check if network has been idle (no pending requests) for threshold seconds.

		Args:
			target_id: The target ID to check
			idle_threshold: Seconds without activity to consider stable

		Returns:
			True if network is stable, False if requests are pending
		"""
		if target_id not in self._pending_requests:
			return True
		if len(self._pending_requests[target_id]) > 0:
			return False
		# Check last activity time
		last_activity = self._last_activity.get(target_id, 0)
		elapsed = asyncio.get_event_loop().time() - last_activity
		return elapsed >= idle_threshold

	async def wait_for_network_stable(
		self,
		target_id: TargetID,
		timeout: float = 30,
		...
		"""Wait until network is stable or timeout.

		Args:
			target_id: The target ID to wait for
			timeout: Maximum seconds to wait
			stable_duration: Seconds of stability required
			check_interval: How often to check (seconds)

		Returns:
			True if network became stable, False if timeout
		"""
		start = asyncio.get_event_loop().time()
		while (asyncio.get_event_loop().time() - start) < timeout:
			if self.is_network_stable(target_id, idle_threshold=stable_duration):
				return True

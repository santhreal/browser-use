			...
			if not element_node.backend_node_id or element_node.backend_node_id == 0:
				...
				if is_sensitive:
					if sensitive_key_name:
						self.logger.info(f'⌨️ Typed <{sensitive_key_name}> to the page (current focus)')
					else:
						self.logger.info('⌨️ Typed <sensitive> to the page (current focus)')
				else:
					self.logger.info(f'⌨️ Typed "{text}" to the page (current focus)')
				...
				try:
					...
					if is_sensitive:
						if sensitive_key_name:
							self.logger.info(f'⌨️ Typed <{sensitive_key_name}> into element with index {index_for_logging}')
						else:
							self.logger.info(f'⌨️ Typed <sensitive> into element with index {index_for_logging}')
					else:
						self.logger.info(f'⌨️ Typed "{text}" into element with index {index_for_logging}')
					self.logger.debug(f'Element xpath: {element_node.xpath}')
					...
					if is_sensitive:
						if sensitive_key_name:
							self.logger.info(f'⌨️ Typed <{sensitive_key_name}> to the page as fallback')
						else:
							self.logger.info('⌨️ Typed <sensitive> to the page as fallback')
					else:
						self.logger.info(f'⌨️ Typed "{text}" to the page as fallback')

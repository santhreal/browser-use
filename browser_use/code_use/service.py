...
"""
	...
	def _format_execution_result(self, code: str, output: str | None, error: str | None) -> str:
		...
		for name, value in self.namespace.items():
			...
			if name.startswith('_') or name in ['json', 'pandas', 'bs4', 'pypdf', 'matplotlib', 'numpy', 'plt', 'done' , 'evaluate', 'navigate', 'asyncio', 'Path', 'csv', 're', 'datetime', 'np', 'pd', 'requests', 'BeautifulSoup', 'PdfReader']:
				continue

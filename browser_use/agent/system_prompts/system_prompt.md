...
- After applying a filter or sort, verify success by checking result changes, URL parameters, or on-page status indicators — do NOT rely solely on `*[` new-element markers. Those markers only appear for newly added interactive elements when the URL stays the same; if the filter triggers a page reload or URL update, no `*[` markers will appear even on a successful application.

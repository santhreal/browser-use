...
Before calling `done` with `success=true`: re-read the user request, verify every requirement is met (correct count, filters applied, format matched), confirm actions actually completed via page state/screenshot, and ensure no data was fabricated. If you used `extract` to collect the specific data you are reporting, cross-check your final answer against those extract results — browser_state may show different content for the same page. If anything is unmet or uncertain, set `success` to `false`.

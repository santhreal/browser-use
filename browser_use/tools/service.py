		...
		async def execute_js(params: ExecuteCDPAction, browser_session: BrowserSession):
			...
			try:
				result = await cdp_session.cdp_client.send.Runtime.evaluate(
					params={'expression': params.js_code}, session_id=cdp_session.session_id
				)
				...
				return ActionResult(
					...
					long_term_memory=f'Executed JavaScript: {params.js_code[:100]}...'
					if len(params.js_code) > 100
					else f'Executed JavaScript: {params.js_code}',
				)
			except Exception as e:

...
class TestCollapsedFilterPanel:
	...
	async def test_collapsed_button_is_interactive_and_shows_expanded_false(self, browser_session, tools, httpserver):
		...
		# Must verify specifically aria-expanded=false, not just that aria-expanded attribute exists.
		# Checking 'aria-expanded' alone passes for both expanded=true and expanded=false.
		assert 'aria-expanded=false' in state_text, (
			'Collapsed state (expanded=false) not reflected in browser state — agent will not know to click the button to expand filters'
		)

	async def test_clicking_more_filters_reveals_filter_options(self, browser_session, tools, httpserver):
		...
		initial_state_text = str(initial_state)
	...
	async def test_applying_revealed_filter_updates_page(self, browser_session, tools, httpserver):
...
class TestHiddenSortPanel:
	...
	async def test_sort_button_shows_expanded_false(self, browser_session, tools, httpserver):
		...
		# Must verify specifically aria-expanded=false, not just that aria-expanded attribute exists.
		assert 'aria-expanded=false' in state_text, 'Sort button collapsed state not reflected in browser state'

	async def test_clicking_sort_reveals_options_and_selection_works(self, browser_session, tools, httpserver):
		...
		# Click the sort option through browser interaction — not via JS evaluate, which would bypass
		# the clickability check and allow the test to pass even when the agent cannot reach the option.
		selector_map = await browser_session.get_selector_map()
		option_index = None
		for idx, element in selector_map.items():
			if 'Price: Low to High' in element.get_all_children_text():
				option_index = idx
				break
		assert option_index is not None, 'Price: Low to High option not clickable after expanding sort panel'

		await tools.click(index=option_index, browser_session=browser_session)

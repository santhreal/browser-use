...
- `is_source_api` (default: `None`): Indicates if the agent is being called from the API (affects default behavior)
- `display_files_in_done_text` (default: `True` for API calls, `False` for UI): Show file information in completion messages

...
- `summarize_every_n_steps` (default: `25`): Summarize the conversation history every N steps. Creates overlapping summaries while keeping recent steps raw. [Example](https://github.com/browser-use/browser-use/blob/main/examples/features/conversation_summarization.py)
- `summarize_keep_last_n_steps` (default: `5`): Number of most recent steps to keep unsummarized when creating summaries. Ensures the agent always has access to recent context.

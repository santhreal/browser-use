...
async def playwright_fill_form(params: PlaywrightFillFormAction, browser_session: BrowserSession):
	...
	try:
		...
		return ActionResult(
			extracted_content=success_msg, long_term_memory=f'Filled form with: {form_data}'
		)
...
async def playwright_screenshot(params: PlaywrightScreenshotAction, browser_session: BrowserSession):
	...
	try:
		...
		return ActionResult(
			extracted_content=success_msg, long_term_memory=f'Screenshot saved: {params.filename}'
		)
